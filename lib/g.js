   ...
   function initEventHandlers(canvas) {
	 ...
	 return keyCode;
      ...
      canvas.onmousedown = function(event) {
         ...
         // if (VR_initWaitCount < 7) {
         //    VR_initWaitCount++;
         // }
         //else if (VR_initWaitCount == 7) {
            //if (!BLOCK) {
               findVRDisplay();
               //findVRDisplay();
            //}
...
//    + " <input id='soundfileinput' type='file' style='visibility:hidden' /> </input>"
      ...
      webgl_canvas.height = height();
       ...
       c2 = document.getElementsByTagName("canvas");
      console.log("canvas:" + c2);
	  
	  	  // zhenyi webvr
	  frameData = new VRFrameData();

		poseStatsSection = document.querySelector('section');
		poseStatsDisplayed = true;
		poseStatsSection.style.visibility = 'visible';

		posStats = document.querySelector('.pos');
		orientStats = document.querySelector('.orient');
		linVelStats = document.querySelector('.lin-vel');
		linAccStats = document.querySelector('.lin-acc');
		angVelStats = document.querySelector('.ang-vel');
		angAccStats = document.querySelector('.ang-acc');
         ...
         navigator.getVRDisplays().then(function(displays) {
           ...
           if(displays.length > 0) {
             console.log('[hehe] Display found');
   		    ...
   		    //interval = setInterval(startWebVR(vrDisplay, webgl_canvas), 500);
   		    startWebVR(vrDisplay, webgl_canvas);
   		}
   	  })
   	}
      else {
